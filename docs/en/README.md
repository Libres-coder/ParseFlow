# ParseFlow Documentation Center

[中文](../README.md) | **English**

Welcome to the ParseFlow documentation!

## 📖 User Guides

- [Quick Start](guides/quick-start.md) - Get started with ParseFlow in 5 minutes
- [FAQ](guides/faq.md) - Frequently asked questions and troubleshooting
- [Examples](guides/examples.md) - Code examples and best practices

## ⚙️ Setup Guides

- [Windsurf Setup](setup/windsurf.md) - Configure ParseFlow for Windsurf IDE (Recommended)
- [Cursor Setup](setup/cursor.md) - Configure ParseFlow for Cursor IDE

## 🛠️ Development Documentation

- [Development Guide](development/development.md) - How to contribute to ParseFlow
- [Architecture](development/architecture.md) - System architecture overview
- [API Reference](development/api.md) - Complete API documentation
- [Naming Conventions](development/naming-conventions.md) - Code and file naming standards

## 📋 Project Planning

- [TODO](planning/todo.md) - Feature roadmap and task list
- [Distribution Analysis](planning/distribution-analysis.md) - Release and distribution plans

## 🔗 Other Resources

- [Project Homepage](../../README_EN.md)
- [Contributing](../../CONTRIBUTING.md)
- [Changelog](../../CHANGELOG.md)
- [License](../../LICENSE)

---

**Tip**: If you're new to ParseFlow, start with the [Quick Start](guides/quick-start.md) guide.

---

## 📝 Documentation Status

| Document          | Status         | Language |
| ----------------- | -------------- | -------- |
| Quick Start       | 🚧 In Progress | English  |
| FAQ               | 🚧 In Progress | English  |
| Windsurf Setup    | 🚧 In Progress | English  |
| Cursor Setup      | 🚧 In Progress | English  |
| API Reference     | 🚧 In Progress | English  |
| Examples          | 🚧 In Progress | English  |
| Architecture      | 🚧 In Progress | English  |
| Development Guide | 🚧 In Progress | English  |

**Note**: English documentation is currently being translated. Chinese versions are available in [docs/](../README.md).

---

## 🌍 Language Support

ParseFlow documentation is available in:

- 🇨🇳 **Chinese** (中文) - [docs/](../README.md) - ✅ Complete
- 🇺🇸 **English** - [docs/en/](README.md) - 🚧 In Progress

---

## 📮 Help & Support

If you need help:

1. Check the [FAQ](guides/faq.md) for common questions
2. Review the documentation relevant to your issue
3. Submit an [Issue](https://github.com/Libres-coder/ParseFlow/issues) on GitHub

---

**Last Updated**: 2025-12-17  
**Version**: 1.6.0
